...
namespace atn {
	...
	namespace {
		...
		size_t inline_set_rules(std::shared_ptr<grammar_atn> const& atn)
		{
			...
			for (size_t state_number = 0; state_number < atn->states().size(); state_number++)
			{
				...
				for (size_t i = 0; i < state->optimized_transitions().size(); i++) {
					...
					std::shared_ptr<atn_state> intermediate_state(std::make_shared<basic_state>(atn->states().size(), target->rule_index()));
		...
		size_t optimize_sets(std::shared_ptr<grammar_atn> const& atn, bool preserve_order)
		{
			...
			for each (std::shared_ptr<decision_state> const& decision in decisions)
			{
				...
				std::shared_ptr<atn_state> set_optimized_state(std::make_shared<basic_state>(atn->states().size(), decision->rule_index()));
		...
		std::shared_ptr<atn_state> state_factory(atn_state::atn_state_type type, size_t state_number, size_t rule_index)
		{
			switch (type)
			{
			...
			case atn_state::atn_state_type::basic:
				return std::make_shared<basic_state>(state_number, rule_index);

			case atn_state::atn_state_type::rule_start:
				return std::make_shared<rule_start_state>(state_number, rule_index);

			case atn_state::atn_state_type::block_start:
				return std::make_shared<basic_block_start_state>(state_number, rule_index);

			case atn_state::atn_state_type::plus_block_start:
				return std::make_shared<plus_block_start_state>(state_number, rule_index);

			case atn_state::atn_state_type::star_block_start:
				return std::make_shared<star_block_start_state>(state_number, rule_index);

			case atn_state::atn_state_type::token_start:
				return std::make_shared<tokens_start_state>(state_number, rule_index);

			case atn_state::atn_state_type::rule_stop:
				return std::make_shared<rule_stop_state>(state_number, rule_index);

			case atn_state::atn_state_type::block_end:
				return std::make_shared<block_end_state>(state_number, rule_index);

			case atn_state::atn_state_type::star_loop_back:
				return std::make_shared<star_loopback_state>(state_number, rule_index);

			case atn_state::atn_state_type::star_loop_entry:
				return std::make_shared<star_loop_entry_state>(state_number, rule_index);

			case atn_state::atn_state_type::plus_loop_back:
				return std::make_shared<plus_loopback_state>(state_number, rule_index);

			case atn_state::atn_state_type::loop_end:
				return std::make_shared<loop_end_state>(state_number, rule_index);
	...
	std::shared_ptr<grammar_atn> atn_deserializer::deserialize(std::vector<uint16_t> const& raw_data) const
	{
		...
		for (size_t i = 0; i < nstates; i++)
		{
			...
			std::shared_ptr<atn_state> state(state_factory(state_type, atn->states().size(), rule_index));
		...
		if (deserialization_options.generate_rule_bypass_transitions() && atn->grammar_type() == atn_type::parser)
		{
			...
			for (size_t i = 0; i < atn->_rule_start_states.size(); i++)
			{
				std::shared_ptr<basic_block_start_state> bypass_start(std::make_shared<basic_block_start_state>(atn->states().size(), i));
				atn->add_state(bypass_start);

				std::shared_ptr<block_end_state> bypass_stop(std::make_shared<block_end_state>(atn->states().size(), i));
				...
				std::shared_ptr<atn_state> match_state(std::make_shared<basic_state>(atn->states().size(), i));
